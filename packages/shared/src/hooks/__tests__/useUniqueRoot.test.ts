...
describe('useUniqueRoot', () => {
  test('basic', async () => {
    ...
    expect(hook.result.current[0].current).toBeInTheDocument();
    const root = hook.result.current[0].current;

    act(() => hook.result.current[1](1));
    expect(times).toBe(2);
    expect(root).toBe(hook.result.current[0].current);
  });
  ...
  test('exists', async () => {
    ...
    expect(root).toBe(hook.result.current.current);
  });
